...
def _parse_exposure_frame(frame: pd.DataFrame) -> pd.DataFrame:
    ...
    result = frame[[ticker_col, exposure_col]].rename(columns={ticker_col: "Ticker", exposure_col: "Exposure"})
    result["Ticker"] = result["Ticker"].astype(str).str.strip().str.upper()
    result["Exposure"] = pd.to_numeric(result["Exposure"], errors="coerce").fillna(0)
    result = result[result["Ticker"] != ""]
    return result.groupby("Ticker", as_index=False)["Exposure"].sum()
...
page_heading()
st.markdown("<div class='method-card'><div class='method-title'>運作流程說明</div><div class='method-desc'>系統自動抓取多個新聞來源，比對公司列表並去除重複新聞，接著以 AI (FinBERT) 模型分析新聞情緒，篩出可能影響公司之負面消息，並依規則分類事件等級。<br>若有上傳曝險部位，會進一步標出曝險高、事件嚴重的公司，同時比對歷史資料新聞頻率異常，最後彙整成圖表與 Excel 檔。</div></div>", unsafe_allow_html=True)
...
if saved_crawl and saved_crawl["event_path"] and saved_crawl["event_path"].is_file():
            ...
            if negative_df.empty:
                ...
                if EXPOSURE_PATH.is_file():
                    ...
                    exposure_map = exposure_df.set_index("Ticker")["Exposure"].to_dict()
                else:
                    exposure_map = {}
